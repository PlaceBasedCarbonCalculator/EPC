# Read in the raw Scottish non-domestic EPC data (one CSV per quarter)
# and save as a single Rds file.
# Input:  ../inputdata/epc/Scotland_NonDomestic_EPC_data_*.zip
#         (from https://www.scottishepcregister.org.uk/)
# Output: epc_scotland_nondomestic_all_raw.Rds
...
unzip(file.path(path,"Scotland_NonDomestic_EPC_data_2014-2025Q2.zip"),
...
for(i in seq(1, length(files_certs))){
  ...
  if(inherits(certs$`Lodgement Date`,"character")){
    certs$`Lodgement Date` <- lubridate::ymd_hms(certs$`Lodgement Date`)
  }
  certs_all[[i]] = certs
