# AUTO-GENERATED from R/clean_epc.R (lines 82-979), with one bug fix (see below).
# Shared free-text standardisation vocabulary + valid-value vectors for the GB
# DOMESTIC EPC data (used by both England/Wales and Scotland clean steps).
#
# Sourced into the global environment by run_domestic_dictionaries() (see
# R/tar_functions.R) after a global `certs` data frame has been prepared.
# Relies on the td_*/sub_*/validate helpers in R/functions.R.  The validate()
# calls are NOT here - validate_domestic() runs them after the 2026 additions
# in R/domestic_vocab_2026_additions.R, so new vocabulary can be added there.
#
# PHOTO_SUPPLY recoding differs between England/Wales and Scotland and is done
# in the clean wrappers, not here.
#
# Do not edit by hand - regenerate with scratchpad/gen_dict.R.
# BUGFIX applied: the single sub_mainheat_description(c(...)) vector call in
# clean_epc.R only removed its first pattern (base gsub uses pattern[1] and
# warns); it is expanded here into four scalar calls.
